...
public class AlbumDAO {
	...
	public List<AlbumVO> searchAlbumsForDropdown(String query) {
		try (SqlSession session = MybatisConfig.getSqlSessionFactory().openSession()) {
			return session.selectList("com.AlbumArchive.mybatis.AlbumMapper.searchAlbumsForDropdown",
					"%" + query + "%");
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public void updateAlbumQty(int albumNum, int qty) {
		try (SqlSession session = MybatisConfig.getSqlSessionFactory().openSession(true)) {
			...
			session.update("com.AlbumArchive.mybatis.AlbumMapper.updateAlbumQty", new HashMap<String, Object>() {
				{
	...
	public boolean deleteAlbum(int albumNum) {
		try (SqlSession session = MybatisConfig.getSqlSessionFactory().openSession()) {
			System.out.println("albumNum = " + albumNum);
			int result = session.delete("com.AlbumArchive.mybatis.AlbumMapper.deleteAlbum", albumNum);
			if (result > 0) {
				session.commit();
				return true;
			}
			...
			System.out.println("deleteAlbum 에러");
			e.printStackTrace();
			return false;
		}
	}
